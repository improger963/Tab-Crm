        ...
        >
          ...
          <div className="px-5 py-3.5 bg-ink-inverse text-white flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 id="receipt-title" className="font-semibold text-sm tracking-tight">POS Ջերմային Կտրոն (80mm Thermal Receipt)</h3>
                <p className="text-xs text-white/70">Պատվեր #{order.id} • {order.purchaseDate}</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="p-4 sm:p-6 overflow-y-auto flex-1 bg-slate-100/70">
            ...
            >
              ...
              <div className="text-center pb-3 border-b-2 border-dashed border-slate-900 space-y-1 receipt-header">
                ...
                <p className="text-2xs text-slate-600 font-medium">Երևան, Հայաստան • Հեռ․ +374 10 00-00-00</p>
                
                {/* Prominent Order ID Box */}
                <div className="pt-1">
                  <div className="inline-block bg-ink-inverse text-white font-mono font-black px-3.5 py-1 rounded-md text-xs sm:text-sm tracking-wide shadow-2xs order-id-badge">
                    ՊԱՏՎԵՐ #{order.id}
                  </div>
                </div>
              </div>
              ...
              {/* Order Meta / Dates */}
              <div className="space-y-1 text-xs pb-3 border-b-2 border-dashed border-slate-900 receipt-meta">
              ...
              <div className="space-y-1 pb-3 border-b-2 border-dashed border-slate-900 receipt-table-container">
                <table className="w-full border-collapse text-2xs receipt-table">
                  <thead>
                    <tr className="border-b border-slate-800 text-2xs uppercase font-black text-slate-700">
                      ...
                      return (
                        <tr key={idx} className="align-top">
                          <td className="py-1.5 pr-1">
                            ...
                            {itemDiscount > 0 && (
                              <div className="text-2xs text-emerald-700 font-semibold">
              ...
              <div className="space-y-1 text-xs pt-1 receipt-totals">
                ...
                {order.remainingBalance && order.remainingBalance > 0 ? (
                  <div className="flex justify-between text-slate-900 text-xs font-black">
                    <span>Ենթակա է վճարման:</span>
                    <span className="font-mono">{order.remainingBalance.toLocaleString()} ֏</span>
                  </div>
                ) : null}

                <div className="flex justify-between items-center text-slate-800 text-2xs pt-1.5 border-t border-dashed border-slate-300">
              ...
              <div className="pt-2 text-center space-y-1 receipt-barcode">
                <div className="h-9 flex items-center justify-center gap-1 opacity-90 overflow-hidden" aria-hidden="true">
                  {Array.from({ length: 34 }).map((_, i) => (
                    <div 
                      key={i} 
                      className={`h-full bg-ink-inverse ${i % 4 === 0 ? 'w-1.5' : i % 2 === 0 ? 'w-1' : 'w-0.5'}`} 
                    />
                  ))}
                </div>
                <p className="font-mono text-2xs font-bold text-slate-700 tracking-widest uppercase">
                  *{order.id}*
                </p>
              </div>

              {/* Footer Return Policy & Thank You */}
              <div className="text-center pt-2 text-2xs text-slate-600 space-y-0.5 border-t border-dashed border-slate-900 receipt-footer">
                <p className="font-black text-slate-900">ՇՆՈՐՀԱԿԱԼՈՒԹՅՈՒՆ ԳՆՈՒՄՆԵՐԻ ՀԱՄԱՐ</p>
                <p className="text-2xs">Ապրանքի փոխանակումը կատարվում է կտրոնի առկայությամբ 14 օրվա ընթացքում:</p>
                <p className="text-2xs text-slate-500 pt-0.5">tab.am • POS Վաճառքի և Պատվերների Համակարգ</p>
              </div>
            </div>
          </div>

          {/* Action Buttons */}
          <div className="px-5 py-3.5 bg-surface border-t border-slate-100 flex items-center justify-between gap-3">
            <button
              type="button"
